...
> Separate codebase at `/Users/Shared/client-template/`. Has its own `CLAUDE.md` and `AUDIT.md`.

## What it is

When a client pays $500, we deploy a **standalone site** on their own free Cloudflare account.
This is NOT the main Next.js platform — it's a self-contained package of static HTML/CSS/JS +
a Cloudflare Worker (~6KB API). Zero npm dependencies. Nothing to maintain.

## How it differs from this platform
...
| Templates | `src/lib/templates/` (6 verticals) | Content seeded from `sample-content.json` |
| Hosting | Our Cloudflare account | Client's own free Cloudflare account |

## Workflow: platform → client
...
3. Client pays $500
4. We export their content JSON from D1
5. Seed it into `sample-content.json` in the client template
6. Deploy Worker + Pages to the client's Cloudflare account
7. Hand them their password — they're independent forever

## Client template architecture

```
/Users/Shared/client-template/
  site/                    Static files → Cloudflare Pages
    js/app.js              Site renderer (data-driven, data-edit attributes)
    js/editor.js           Inline WYSIWYG (discovers editable elements via data attributes)
    js/admin.js            12-tab form editor
    js/config.js           API_BASE URL (change per client)
    css/themes.css         Same 4 themes as main platform
    css/styles.css         All site + edit-mode styles
  worker/src/index.js      6 endpoints: content CRUD, login, upload, auth check, image serve
  sample-content.json      Demo content (seeded to KV)
```
...
| `data-edit-image="json.path"` | Upload/replace image |
| `data-edit-list="json.path"` | Add/remove list items |
| `data-visibility="key"` | Section show/hide toggle |

Adding a new editable element = add the attribute in `app.js`. No `editor.js` changes.
This scales to unlimited verticals without editor modifications.

## Client template status

### Done (2026-04-27)
- [x] Public site renders all sections from JSON (same layout as main platform)
- [x] 4 themes working (modern, industrial, luxury, friendly)
- [x] Inline WYSIWYG editor — every text, image, list, and section toggle
- [x] 12-tab form editor
- [x] Worker API: content CRUD, auth, image upload to R2
- [x] Data-driven editor refactor (zero hardcoded selectors)
- [x] Full codebase audit (`AUDIT.md`)
- [x] Project docs (`CLAUDE.md`, `.gitignore`)
- [x] `/admin` renamed to `/mysite` (avoid bot wordlist scanning)
- [x] `admin.js` → `mysite.js`, `editor.js` → `editsite.js`, `admin.css` → `mysite.css`
- [x] Inline editor is default after login (not form editor)
- [x] Fixed "phantom New Service" bug in bindAllLists (guard checked wrong location)
- [x] Git repo set up: `client-template/auto-repair` on GitHub, email `client-template@proton.me`
- [x] Cloudflare resources renamed: Worker `auto-repair-api`, Pages `auto-repair`
- [x] Full A-Z security audit + hardening (see security section below)

### Done (2026-04-27 security hardening)
- [x] `timingSafeEqual` rewritten — HMAC-based fixed-length comparison (eliminates length-leak side channel)
- [x] Separate `TOKEN_SECRET` from login `PASSWORD` for HMAC signing (set as Worker secret)
- [x] Rate limiting on login (5 attempts / 15 min per IP, KV-backed lockout)
- [x] CORS locked to `ALLOWED_ORIGIN` env var (no more wildcard `*`)
- [x] All inline scripts extracted to external files (`login.js`, `boot-public.js`, `boot-editor.js`)
- [x] CSP header via `_headers` file — `script-src 'self'` enforced, zero inline scripts
- [x] SVG removed from upload allow-list (stored XSS vector via malicious SVG)
- [x] 512KB payload limit on POST /api/content (prevents KV abuse)
- [x] `X-Forwarded-For` removed from rate limiter — `CF-Connecting-IP` only (not spoofable)
- [x] Single-quote escaping added to `esc()` in `app.js` and `mysite.js`
- [x] Security headers on all responses: X-Frame-Options DENY, X-Content-Type-Options nosniff, Referrer-Policy, Permissions-Policy
- [x] Path traversal prevention on image endpoint (rejects `..`, requires `uploads/` prefix)
- [x] `.gitignore` fixed for nested `.wrangler/` directories

### Client template security

All 19 audit findings + 5 Red Team simulation findings resolved. Full details in `/Users/Shared/client-template/SECURITY.md`.
...
- [ ] Favicon + meta tags
- [ ] Automate the platform → client export pipeline (step 4-6 above)
- [ ] Test full end-to-end deployment on a real client's Cloudflare account

### Lessons learned
- `contenteditable` on elements containing SVG icons breaks — wrap text in a dedicated `<span>`
- Stats counter animation overwrites `textContent` continuously — must disable in edit mode
- Testimonial carousel replaces innerHTML — must re-bind editor after each advance
- Cache busting (`?v=N`) on all script/link tags is essential during development
- **Always run adversarial security audit after building.** Builder accumulates context blindness — trusts own output after hours of work. Fresh eyes with a specific adversarial mandate catch what the builder misses. The dual-agent red/blue team audit found 19 issues the builder overlooked, including a CRITICAL token flaw the builder wrote himself. Never ship auth/security code without a separate adversarial review pass.

---

# 📒 Change log (reverse chronological)

## 2026-04-27 — Client template: security hardening + path rename + infra cleanup

### Security hardening (`/Users/Shared/client-template/worker/src/index.js`)
- Rewrote `timingSafeEqual` from naive XOR (had length-leak) to HMAC-based fixed-length comparison
- Added `TOKEN_SECRET` env var — HMAC signing key now separate from login password
- Added 512KB payload size limit on POST /api/content with double-check (Content-Length header + body length)
- Removed `X-Forwarded-For` fallback from rate limiter — CF-Connecting-IP only (can't be spoofed behind Cloudflare)
- Removed `image/svg+xml` from upload allow-list — SVG can contain inline JS (stored XSS)
- Set `ALLOWED_ORIGIN` Worker secret to lock CORS to `https://auto-repair.pages.dev`

### CSP + inline script removal
- Extracted login.html inline script → `/js/login.js`
- Extracted index.html inline script → `/js/boot-public.js`
- Extracted edit.html inline script → `/js/boot-editor.js`
- Added `site/_headers` with full CSP: `script-src 'self'` — no inline execution allowed
- Security headers (X-Frame-Options, nosniff, Referrer-Policy, Permissions-Policy) on all Pages responses

### XSS hardening
- Added single-quote (`'` → `&#39;`) escaping to `esc()` in both `app.js` and `mysite.js`

### Path rename + infra
- Renamed `/admin` → `/mysite` across all HTML files, JS references, redirects
- Renamed `admin.js` → `mysite.js`, `editor.js` → `editsite.js`, `admin.css` → `mysite.css`
- Login now redirects to inline editor (`/mysite/edit.html`) by default, not form editor
- Fixed "phantom New Service" bug — `bindAllLists` guard was checking inside container instead of sibling
- Worker renamed to `auto-repair-api`, Pages renamed to `auto-repair` on Cloudflare
- Git repo: `client-template/auto-repair`, email `client-template@proton.me`
- Cache bust v7 → v8 on all HTML resources

## 2026-04-27 — Client template: data-driven editor + audit + docs

### Client template (`/Users/Shared/client-template/`)
...
- Rewrote `app.js` with `data-edit` attributes on every editable element
- Fixed SVG+text contenteditable conflicts (text wrapped in dedicated spans)
- Stats animation + testimonial auto-advance disabled in edit mode
- Fixed CSS `[data-editable]` → `[data-edit]` selector mismatch
- Fixed mojibake in admin.js comments (corrupted Unicode)
- Created `CLAUDE.md`, `AUDIT.md`, `.gitignore` for the client template project
- Cache bust v5 → v6 on all HTML resources
